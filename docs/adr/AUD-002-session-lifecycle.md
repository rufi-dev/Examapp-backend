...
- **Classifying a `/auth/refresh` request — full precedence (ADR-017).** Evaluate **top to bottom**; the first match wins. Only one outcome is confirmed theft; a stale cookie from an ordinary logout/reset must never be counted or paged as theft.
  ...
  7. **Immediately-previous generation within grace (idempotent Contract A)** — `gen == currentGen - 1`, `hash(secret)` matches the ring entry for that gen, and `now - lastRotatedAt <= graceWindow` (10 s). A successful rotation atomically stores a short-lived **encrypted** copy of its exact response, bound to the consumed `{gen,hash}` and the current Session state. A repeat presentation receives that exact recorded pair with `200`; the generation and Session state do not advance. This repairs a hard-reload/lost-response race while preserving exactly one live refresh secret.
     - *Concurrent tabs:* the loser normally adopts the leader's broadcast; if it reaches the server, it receives the same recorded response.
     - *Lost response:* the new page can recover the already-committed rotation instead of being forced to login.
     - If the replay record is missing, expired, malformed, cannot decrypt, or does not bind to the current `sid/gen/hash`, the server keeps the legacy strict `409` behavior and never invents a response.
     - **Rejected alternative — rotating "leeway re-issue":** letting the immediately-previous secret rotate forward once more remains **unsafe** and rejected — each such rotation resets `lastRotatedAt` and advances the generation, enabling leapfrog. Idempotent replay never advances the generation, so it does not create that vulnerability.
  ...
  **Status-code contract:** `200` rotate or exact idempotent replay · `409` grace only when no valid replay record exists · `403` confirmed theft (case 8 only) · `401` everything else that is an auth outcome (cases 0–5, 9) · `5xx` infrastructure (case 10). Observability (§11) must label case 3 as a benign `revoked` outcome and reserve the theft counter/page for case 8.
...
3. Reuse-detection strictness (§2.2): the **10 s** grace window and **N = 10** ring depth are accepted. The immediately-previous generation may receive only the exact encrypted idempotent replay from its committed rotation; it never rotates again. Above the window it is theft. The rotating "leeway" alternative remains rejected.
...
Core invariant the design preserves: **exactly one live refresh secret per session at any instant, and the generation counter advances only via a genuine rotation.** Idempotent replay returns that same live secret and never advances the generation; rotating leeway remains forbidden (§2.2).
