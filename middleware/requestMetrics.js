...
// minuteEpoch -> { count, err4, err5, totalMs, maxMs }  (last 60 min, windowed)
const buckets = new Map();
// "GET /api/quiz/exam/:id" -> { count, totalMs, maxMs, err4, err5, lastAt }
// NOTE: cumulative since process start (NOT windowed) — the frontend labels
// these tables "yenidən başlamadan bəri" (since restart), not "last hour".
...
const requestMetrics = (req, res, next) => {
  // Skip traffic that would make the dashboard measure itself or skew latency:
  //  - /api/health: the page polls every ~30s.
  //  - health check's OWN probes (checkApi hits GET / and /server-time via the
  //    loopback + Caddy): tagged with a distinct User-Agent, else they'd top
  //    the "busiest endpoints" table and arm the 5xx alert gate on an idle app.
  //  - OPTIONS: one CORS preflight per API call would double counts.
  const ua = req.headers["user-agent"] || "";
  if (
    req.method === "OPTIONS" ||
    req.originalUrl.startsWith("/api/health") ||
    ua.startsWith("Examopia-HealthCheck")
  ) {
    return next();
  }
  ...
  res.on("finish", () => {
    try {
      // Streaming responses (SSE — AI PDF extraction/chat) stay open for tens of
      // seconds; recording that as one request's latency would wreck avgMs/maxMs
      // and fire a false "slow API" alert. Skip them from the timing metrics.
