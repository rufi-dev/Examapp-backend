...
- "pairs": for Cma, one object per correct left<->right pair (1:1). Put math inline in "left"/"right" via $...$ and set "leftLatex"/"rightLatex" to "". Empty array otherwise. Each "right" must be a SINGLE option — NEVER comma-join several letters/values into one right (write "a", not "a, d"). Right values must be distinct. Each left maps to exactly one right; if the source shows a left with several letters, pick the single intended match (do not group).
