...
// Shared guard: an archived (trashed) exam is read-only everywhere except the
// Trash restore / permanent-delete actions. Returns true (and responds) if the
// exam is archived, so callers do `if (blockIfArchived(res, exam)) return;`.
...
function isCorrectAnswer(ca, sel) {
  ...
  switch (ca.type) {
    ...
    default: {
      ...
      // (teachers/AI can list several variants), compared case-insensitively with
      // collapsed whitespace. Falls back to the single `answer` for legacy items.
      const accepted =
...
function renderableCorrect(ca) {
  ...
  // Open: show every accepted variant, e.g. "x+2 / x + 2".
...
async function archiveExams(filter) {
  await Exam.updateMany({ ...filter, deletedAt: null }, { $set: { deletedAt: new Date() } });
}
...
const restoreExam = asyncHandler(async (req, res) => {
  ...
  if (parentOk) {
    ...
    // Orphaned (class deleted): need a destination class from the caller. An exam
    // must never be class-less, so restore requires a valid, owned target class.
    if (!classId) {
      return res.status(409).json({
        needsClass: true,
        message:
