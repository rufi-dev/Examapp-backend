...
// CRITICAL: this must NEVER reject — a rejecting probe (e.g. a raw mongoose
// query during an Atlas failover) would otherwise reject the outer Promise.all
// and collapse the ENTIRE payload to {error}, making the dashboard blank
// exactly during an incident. So both the probe's rejection AND the timeout
// resolve to the fallback value.
const withTimeout = (promise, ms, fallback) => {
  const fb = () => (typeof fallback === "function" ? fallback() : fallback);
  const p = Promise.resolve(promise).catch(fb);
  return Promise.race([p, sleep(ms).then(fb)]);
};
...
const timedFetch = async (url, { timeoutMs = 10000, method = "GET", redirect = "follow" } = {}) => {
  const t0 = Date.now();
  try {
...
const dirSizes = () =>
  cached("dirSizes", 30 * 60 * 1000, async () => {
    ...
    // A `du` on a huge tree (.wwebjs_auth Chromium profiles, node_modules) can
    // be slow. On timeout return {bytes:null} instead of dropping the row —
    // otherwise the BIGGEST dirs (the ones that matter for a disk alert) are
    // exactly the ones that silently vanish. maxBuffer raised for long output.
    const duOne = (dir) =>
      new Promise((resolve) => {
        execFile(
          "du",
          ["-sk", path.join(cwd, dir)],
          { timeout: 25000, maxBuffer: 4 * 1024 * 1024 },
          (err, stdout) => {
            if (err) return resolve({ dir, bytes: null });
            const kb = Number(String(stdout).trim().split(/\s+/)[0]);
            resolve({ dir, bytes: Number.isFinite(kb) ? kb * 1024 : null });
          }
        );
      });
    // Whole batch is bounded so it can't blow checkServer's own timeout even if
    // several du calls hang; partial results are fine (cached 30 min).
    const sizes = await withTimeout(
      Promise.all(entries.map(duOne)),
      20000,
      entries.map((dir) => ({ dir, bytes: null }))
    );
    return sizes
...
const checkBusiness = () =>
  cached("business", 60 * 1000, async () => {
    ...
    const [
      ...
      Attempt.countDocuments({ submitted: false }),
      Attempt.countDocuments({ createdAt: { $gte: since24h } }),
    ...
    try {
      ...
      if (withCover?.coverImage && isCloudinaryUrl(withCover.coverImage)) {
        // redirect:"manual" so a whitelisted URL can't 302 into the internal
        // network (belt-and-braces on top of the host allowlist above).
        const r = await timedFetch(withCover.coverImage, { timeoutMs: 8000, method: "HEAD", redirect: "manual" });
...
const checkStorage = () =>
  cached("storage", 5 * 60 * 1000, async () => {
    ...
    // Inventory of uploaded PDFs (count, size, biggest files). The true file
    // COUNT comes from the full readdir (cheap); only stat-ing is capped at
    // 3000 files, and `sampled` flags when the byte total is from that sample
    // so the UI can mark it approximate rather than silently undercount.
    let files = { count: 0, bytes: 0, largest: [], sampled: false };
    try {
      const allNames = await fsp.readdir(dir);
      const SAMPLE = 3000;
      const names = allNames.slice(0, SAMPLE);
      const stats = [];
      for (const name of names) {
        try {
          const st = await fsp.stat(path.join(dir, name));
          if (st.isFile()) stats.push({ name, bytes: st.size });
      ...
      files = {
        ...
        sampled: allNames.length > SAMPLE,
      };
...
// (probing would cost money or isn't meaningful). Nothing here throws, and the
// live probes run in PARALLEL so one hanging provider can't push total time
...
const checkIntegrations = () =>
  cached("integrations", 15 * 60 * 1000, async () => {
    ...
    const telegramProbe = async () => {
      try {
        const { isTelegramConfigured, tgApi } = require("../helper/telegram");
        if (!isTelegramConfigured())
          return { name: "Telegram bot", configured: false, ok: null, detail: "TELEGRAM_BOT_TOKEN qurulmayıb" };
        const t0 = Date.now();
        ...
        return { name: "Telegram bot", configured: false, ok: null, detail: String(e?.message || e).slice(0, 120) };
      }
    };
    ...
    // --- AI providers: configured flags + today's spend (no paid live probe).
    const aiRow = async () => {
    ...
    // Independent probes → run concurrently; each already self-contained/safe.
...
const buildAlertsAndScore = (d) => {
  ...
  // A whole check that timed out or errored comes back as {error:...} (cached
  // fallback) or a withTimeout fallback with no data. Detect that so a hard
  // failure raises an alert instead of silently scoring 100.
  const checkErrored = (x) => !x || x.error != null;

  // ---- site availability (weight ~20)
  const sitePages = d.site?.pages || [];
  if (checkErrored(d.site) || d.site?.pages == null) {
    add("warning", "Sayt", "Sayt yoxlaması tamamlanmadı (vaxt bitdi)", "Serverin internet çıxışını yoxlayın");
    dock(6);
  } else if (sitePages.length > 0 && sitePages.every((p) => !p.ok)) {
    ...
    dock(8);
  } else if (sitePages.some((p) => p.ok && p.ms > T.siteMsWarn)) {
  ...
  const internalOk = internalApi && internalApi.ok;
  ...
  if (publicApi && !publicApi.ok) {
    // If Express answers internally but the full public path doesn't, it's
    // usually Caddy/DNS/cert OR a container-hairpin quirk (the probe reaches
    // its own public IP from inside Docker) — real users may be fine. Downgrade
    // to warning in that case; only call it critical when Express is ALSO down.
    if (internalOk) {
      add("warning", "API", "api.examopia.com serverin daxilindən yoxlanılmadı (Caddy/DNS/SSL və ya konteyner hairpin) — real istifadəçilər təsirlənməyə bilər", "Xaricdən açılışı yoxlayın; problem yalnız serverdən görünürsə, hairpin ola bilər");
      dock(4);
    } else {
  ...
  // Cap the score by status so the headline number can never contradict the
  // banner (e.g. a "down" state showing a green 85/100 "Yaxşı"). The score-band
  // thresholds are: <25 severe, <50 critical, <75 warning, ≥75 good.
  score = Math.max(0, Math.min(100, Math.round(score)));
  if (status === "down") score = Math.min(score, 20);
  else if (status === "critical") score = Math.min(score, 45);
...
const uptimeWindow = async (ms) => {
  // Never reject — a raw-query rejection here must not collapse the whole
  // /api/health payload (the withTimeout wrapper is the second line of defense).
  try {
    ...
    // One interval of grace: the just-due sample lands a few seconds after its
    // boundary (probes take 0.3–10s), so don't count it as "missing" yet —
    // otherwise API% flickers below 100 around every 5-minute boundary.
    const elapsed = Date.now() - new Date(first.at).getTime() - SAMPLE_INTERVAL_MS;
    const expected = Math.max(1, Math.floor(elapsed / SAMPLE_INTERVAL_MS) + 1);
    return {
      expected,
      got,
...
const getHealth = asyncHandler(async (req, res) => {
  ...
  // Defensive: if the payload ever degrades to a bare {error} (a cached failure
  // from a prior tick), report it as 503 so the frontend keeps its last good
  // data + shows a banner instead of rendering a bogus empty "warning" page.
  if (!payload || !payload.overall) {
    res.status(503);
    return res.json({ error: payload?.error || "health unavailable" });
  }
  res.json(payload);
});
